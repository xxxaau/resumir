...
Readability.prototype = {
  ...
  _grabArticle(page) {
    ...
    while (true) {
      ...
      if (textLength < this._charThreshold) {
        ...
        // FIX: Use DOMParser
        try {
            page.replaceChildren(); // Clear
            const parser = new DOMParser();
            const doc2 = parser.parseFromString(pageCacheHtml, "text/html");
            while (doc2.body.firstChild) {
                page.appendChild(doc2.body.firstChild);
            }
        } catch(e) {}
  ...
  _unwrapNoscriptImages(doc) {
    ...
    this._forEachNode(noscripts, function (noscript) {
      ...
      // Use DOMParser to safely parse content
      try {
        const parser = new DOMParser();
        const doc2 = parser.parseFromString(noscript.innerHTML, "text/html");
        
        // Move children from parsed doc to existing tmp div
        while (doc2.body.firstChild) {
            tmp.appendChild(doc2.body.firstChild);
        }
      } catch (e) {
          // Fallback or ignore
      }
