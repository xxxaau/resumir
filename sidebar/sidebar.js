...
- **Cites Destacades**: Màxim 3 frases literals del text original (traduïdes si cal, o en idioma original si és més precís, però explicades).`;
  ...
  function setGeneratingState(generating) {
      ...
      if (generating) {
          ...
          copyBtn.disabled = !currentMetadata.summary;
      }
  }
  ...
  async function startSummary(overrideText = null) {
    ...
    try {
      // 1. Get Configuration
      const config = await browser.storage.local.get(["apiKey", "modelName", "systemPrompt"]);
  ...
  async function loadModels(apiKey, currentModel) {
    ...
    try {
        ...
        const validModels = data.models
            ...
            .filter(name => name.includes("gemini") || name.includes("gemma"));
  ...
  async function getPageContent() {
      ...
      // YOUTUBE SPECIAL LOGIC
          ...
          try {
              // 1. Get Caption Track URL from Main World (Accessing YouTube Player API)
              const playerResponse = await browser.scripting.executeScript({
                  target: {tabId: tabId},
                  world: "MAIN", // Access the page's window object
                  func: () => {
                      try {
                          // Try getting the player instance (works for SPA navigation too)
                          const player = document.getElementById('movie_player');
                          const response = player && player.getPlayerResponse ? player.getPlayerResponse() : (window.ytInitialPlayerResponse || null);
                          ...
                          // Prefer Catalan, then English, then first available
                          // Note: tracks usually have 'languageCode'
                          let track = tracks.find(t => t.languageCode === 'ca') || 
                                      tracks.find(t => t.languageCode === 'en') || 
                                      tracks[0];
                                      
                          return { baseUrl: track.baseUrl, language: track.name?.simpleText };
                      } catch (e) {
                          return null;
                      }
                  }
              ...
              if (trackData && trackData.baseUrl) {
                   console.log("Found transcript track:", trackData.language);
                   ...
                   try {
                       ...
                       // 3. Parse XML (Simple Regex or DOMParser)
                       // Sidebar background context has DOMParser
                       const parser = new DOMParser();
                       ...
                       for (let i = 0; i < texts.length; i++) {
                           // Decode HTML entities (XML parser does this mostly, but let's be safe)
                       ...
                       // Add metadata header
                       text = `[TRANSCRIPT (${trackData.language})]\n\n${fullText}`;
                       
                   } catch (err) {
                       console.error("Error fetching/parsing transcript:", err);
                       // Fallthrough to fallback
                   }
              }
              
              // Fallback: If no internal API access or fetch failed, try the visible panel again
              if (!text || text.trim() === "") {
                   const transcriptResult = await browser.scripting.executeScript({
                       target: {tabId: tabId},
                       func: () => {
                           // Try new UI selector (Feb 2025)
                           const segments = document.querySelectorAll('ytd-transcript-segment-renderer .segment-text, .ytd-transcript-segment-renderer .segment-text');
                   ...
                   if (transcriptResult?.[0]?.result) {
                       text = "TRANSCRIPT (Panel): " + transcriptResult[0].result;
                   }
              }
          } catch (e) {
